...
CREATE OR REPLACE FUNCTION MEDIA_TRABAJADA
RETURN NUMBER
IS
    TOTAL_ANOS NUMBER;
    NUM_TRABA NUMBER;
...
BEGIN
    SELECT SUM (EDAD_REAL(FECHA_NACIMIENTO)),COUNT(CODIGO) INTO TOTAL_ANOS, NUM_TRABA
    FROM TRABAJADORES;
    
    MEDIA:=TOTAL_ANOS/NUM_TRABA;
...
BEGIN 
	...
	EXCEPTION	
		WHEN NO_DATA_FOUND THEN
			DBMS_OUTPUT.PUT_LINE ('NO SE HA ENCONTRADO EL ABOGADO ' || NUM_ABO);
END;
/
...
BEGIN     
        ...
        FROM DEMANDANTE 
        WHERE DNI_DEMANDANTE = DNI_DE;
        
        IF CONTADOR = 0 THEN
            RAISE_APPLICATION_ERROR (-20050, 'NO SE HA ENCONTRADO EL DEMANDANTE ' || DNI_DE);
        END IF;

		SELECT NOMBRE INTO NOMBRE_ABOGADO
		FROM TRABAJADORES
		WHERE CODIGO = NUM_ABO;
		
        DBMS_OUTPUT.PUT_LINE('-----------------------');
        DBMS_OUTPUT.PUT_LINE('EL ABOGADO: ' || NUM_ABO || '(' || NOMBRE_ABOGADO || ') SE REUNIO CON EL DEMANDANTE: ' || dni_de);
        DBMS_OUTPUT.PUT_LINE('-----------------------');
...
BEGIN
    ...
    END LOOP;
        CONTADOR := CONTADOR-1;
		DBMS_OUTPUT.PUT_LINE('Total casos: ' || CONTADOR);
