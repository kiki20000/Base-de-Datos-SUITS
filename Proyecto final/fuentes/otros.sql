...
CREATE OR REPLACE VIEW DATOS_ESTUDIANTES
AS 
    SELECT CODIGO_EXPEDIENTE, C.DNI, C.NOMBRE, C.APELLIDOS
    FROM CLIENTE C, EXPEDIENTE E 
    WHERE E.CLIEN_DNI =  C.DNI;

CREATE OR REPLACE VIEW TOTAL_COMISIONES
AS 
    SELECT SUM(COMISIONES) AS TOTAL
    FROM PENAL;
